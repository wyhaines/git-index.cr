...
class GitIndex
  @config : NamedTuple(String, String)
  ...
  def list_records(db)
    puts "hash,path,url"
    db.query("SELECT hash, path, url FROM repositories") do |row|
      puts row.join(",")
    end
  end
  ...
  def query_records(db)
    ARGV.each do |hash_or_url|
      db.query("SELECT hash, path, url from repositories WHERE hash like ? or url = ?", ["#{hash_or_url}%"], [hash_or_url]) do |row|
